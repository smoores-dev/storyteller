...
Version is either `latest` or `web-<version>` (eg `web-2.7.0`). Variant is one
of `cuda-11.8`, `cuda-12.9`, `cuda-13.1`, `rocm`, `sycl`, `blas`, or `vulkan`.
If you don't specify a variant, the image defaults to CPU-only.
...
`nvidia-smi`): `cuda-13.1` for CUDA 13.1+, `cuda-12.9` for CUDA 12.x, or
`cuda-11.8` for CUDA 11.x.
...
services:
  web:
    image: registry.gitlab.com/storyteller-platform/storyteller:latest-cuda-13.1
    runtime: nvidia
    volumes:
