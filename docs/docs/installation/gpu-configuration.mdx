...
[self-hosting](/docs/installation/self-hosting#file-ownership-on-linux-puid-and-pgid).
...
[ghost-story](/docs/tutorials/offloading-transcription).
...
This variant is only recommended for discrete Intel GPUs, eg an Intel Arc or
Intel Data Center graphics card. The SYCL variant is likely _slower_ than the
CPU variant when trying to use it for an iGPU! Please use the CPU variant
