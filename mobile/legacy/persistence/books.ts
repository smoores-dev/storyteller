...
import { type ReadiumManifest, areLocatorsEqual } from "@/modules/readium"
import {
  type ReadiumLocator,
...
export async function readLocator(bookId: number) {
  const locatorString = await AsyncStorage.getItem(`books.${bookId}.locator`)
  if (!locatorString) return null
  return JSON.parse(locatorString) as TimestampedLocator
}
...
export async function writeHighlight(bookId: number, highlight: string) {
