...
CREATE TABLE IF NOT EXISTS "author_to_book" (
  ...
  author_uuid TEXT,
  role TEXT,
  FOREIGN KEY (book_uuid) REFERENCES book (uuid) FOREIGN KEY (author_uuid) REFERENCES author (uuid)
);
...
CREATE TABLE token_revokation (token TEXT PRIMARY KEY);
...
CREATE TABLE position(
  ...
  FOREIGN KEY (user_uuid) REFERENCES user (uuid) FOREIGN KEY (book_uuid) REFERENCES book (uuid) UNIQUE (user_uuid, book_uuid)
);
